...
const SongEncounterDirectorScript := preload(
	"res://src/song_encounter_director.gd"
)

var song_encounter_director := SongEncounterDirectorScript.new()
...
func initialize() -> void:
	...
	song_encounter_director.reset()

	world_plan = WorldPlan.new(WorldPlan.WorldType.NORMAL)
	Log.i("World world_plan created: %s" % world_plan)

	player = MonsterFactory.create_monster(
		&"knight",
		Roles.Type.KNIGHT
	)

	# Do not give the player the normal Knight starting equipment.
	# Roles.equip_monster(player, Roles.Type.KNIGHT)

	# Begin with 20% of the normal Knight's physical combat statistics.
	player.max_hp = maxi(1, int(round(player.max_hp * 0.20)))
	player.hp = player.max_hp

	player._base_strength = maxi(
		1,
		int(round(player._base_strength * 0.20))
	)

	player.intelligence = maxi(
		1,
		int(round(player.intelligence * 0.20))
	)

	Log.i(
		"Player created weak: HP %d/%d, STR %d, INT %d, AC %d"
		% [
			player.hp,
			player.max_hp,
			player.get_strength(),
	...
	var song_title: String = MusicManager.get_current_song_title()

	if song_title.is_empty():
		song_title = MusicManager.play_random_song()
	

	var encounter: Dictionary = song_encounter_director.populate_from_song(
	map,
	player,
	song_title
)

	Log.i("Initial song encounter: %s" % encounter)
